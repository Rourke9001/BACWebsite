...
|---|---:|---:|---:|---:|---:|
| Sequential, 6 targets × 3 hostnames, 07:24–07:52 UTC | 295 | 0 | 0.92 s | 2.49 s | 2.75 s |
| Sequential, `/inc/css/main.css` @ 2 s | 182 | 0 | 0.72 s | 1.55 s | 2.21 s |
| Sequential, `/` @ 2 s | 169 | 0 | 0.88 s | 1.39 s | 1.71 s |
| **Concurrent** — 25 × full 23-resource page load | 598 | 0 | 0.55 s | 2.86 s | **8.98 s** |

**1,374 requests, zero 502s, zero 5xx.** The failure is rarer than that.

### The Functions host is not in the static path — settled

- 776 sequential static-asset requests produced **zero** entries in Application Insights,
...
- Sequential probing never exceeded **2.75 s** in 776 samples.
...
that 1,374 requests missed it. Not falsifiable with the evidence available today —
